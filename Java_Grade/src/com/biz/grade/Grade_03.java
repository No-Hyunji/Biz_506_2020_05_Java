...
public class Grade_03 {
	...
	public static void main(String[] args) {
		...
		int subCount = 5 + 2; // 과목 5개의 총점, 평균을 저장할 배열

		// 값으로 배열 선언하기
		// 배열을 값으로 선언 할 때는 {} 끝에 반드시 ;를 붙여라
		...
		for (int i = 0; i < stdCount; i++) {
			...
			// 총점항목과 평균 항목을 제외하고 과목 부분만 점수를 채워넣기
		...
		for (int i = 0; i < stdCount; i++) {
			...
			for (int j = 0; j < subCount - 2; j++) {
				...
				//이 부분에 놓으면 문제를 일으킬 수 있음score[i][subCount - 1]= score[i][subCount-2]/(subCount-2);
		...
		/*
		 * 배열.length :배열의 개수가 몇 개 인지 알려주는 키워드 변수, 필드변수,맴버변수 배열의 개수가 궁금할 때 일일이 세어보지 않아도 알
		 * 수 있게 해주는 키워드 변수이다.
		 */
